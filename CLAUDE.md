...
- `evict_and_move` doesn't honor `target_node` (the "move" is best-effort), though it now evicts via the PDB-respecting Eviction API; `patch_limits` is the only multi-container-aware action.
